> ## This is a fork
>
> Unofficial build of **[AlmanacClassSystem](https://thunderstore.io/c/valheim/p/RustyMods/AlmanacClassSystem/)
> 0.6.9 by RustyMods**, rebuilt so it loads on **Valheim 1.0** (Unity 6). Gameplay is untouched - only the
> API calls the game renamed or re-signed were fixed.
>
> All credit for the mod goes to RustyMods and the contributors listed below. The changes are offered back
> upstream as [pull request #31](https://github.com/RustyMods/AlmanacClasses/pull/31); once that is merged and
> released, **use the original and drop this fork**.
>
> Fork source: https://github.com/PawelSzymanski89/AlmanacClasses (branch `valheim-1.0`)
>
> ### What was fixed for 1.0
> - Project rebuilt as SDK-style `netstandard2.1` / C# 12 (the old .NET 4.8 project no longer compiles).
> - `ServerSync` compiled from source instead of the bundled `ServerSync.dll` - `ZRoutedRpc.Everybody`
>   became a `const` in 1.0, and a `const` read is baked in at compile time, so the prebuilt binary threw
>   `MissingFieldException`.
> - `Trader`: `Inventory.IsTeleportable()` now takes a parameter.
> - `Pickpocket`: `CharacterDrop.DropItems(...)` gained a `cheated` flag.
> - `TalentBook`: implements the new `Hoverable.GetHoverOffset()`.
> - `PieceManager`: reads `m_availablePiecesByCategory` (`m_availablePieces` became a `HashSet`).
> - `MasterChef` / `RainProof`: pinned Harmony signatures updated for `GetTooltip` and `AddStatusEffect`.
